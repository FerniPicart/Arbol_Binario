from TDA_Pila_Din import Pila, apilar, pila_vacia, desapilar
from TDA_Cola import Cola, cola_vacia, arribo, atencion
from TDA_Heap import Heap, arribo_heap, atencion_heap, busqueda_heap_aero, heap_vacio, cambiar_prioridad, busqueda_heap, busqueda_heap_red, busqueda_heap_antena
...
def insertar_vertice_antena(grafo, dato):
    '''Inserta un vértice al grafo'''
    nodo = Nodo_Vertice(dato)
    if grafo.inicio is None or grafo.inicio.info.id > dato.id:
        nodo.sig = grafo.inicio
        grafo.inicio = nodo
    else:
        ant = grafo.inicio
        act = grafo.inicio.sig
        while act is not None and act.info.id < nodo.info.id:
            ant = act
            act = act.sig
        nodo.sig = act
        ant.sig = nodo
...
def insertar_arista_puesto(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_puesto(origen.adyacentes, dato, destino.info.nombre)
    if not grafo.dirigido:
        agregrar_arista_puesto(destino.adyacentes, dato, origen.info.nombre)
...
def insertar_arista_antena(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_antena(origen.adyacentes, dato, destino.info.nombre)
    if not grafo.dirigido:
        agregrar_arista_antena(destino.adyacentes, dato, origen.info.nombre)


def agregrar_arista_antena(origen, dato, destino):
    '''Agrega la arista desde el vértice origen al destino'''
    nodo = Nodo_Arista(dato, destino)
    if origen.inicio is None or origen.inicio.destino > destino:
        nodo.sig = origen.inicio
...
def barrido_grafo_antena(grafo):
    '''Realiza un barrido de la grafo mostrando sus valores'''
    aux = grafo.inicio
    while aux is not None:
        print('vertice:',aux.info.id,'|',aux.info.nombre)
        print('adyacentes:')
        adyacentes(aux)
        print()
        aux = aux.sig
...
def buscar_vertice_nombre(grafo, buscado):
    '''Devuelve la direccion del elemento buscado'''
    aux = grafo.inicio
    while aux is not None and aux.info.nombre != buscado:
        aux = aux.sig
...
def buscar_vertice_id(grafo, buscado):
    'Devuelve la direccion del elemento buscado'
    aux = grafo.inicio
    while aux is not None and aux.info.id != buscado:
        aux = aux.sig
    return aux
...
def barrido_profundidad_aero(grafo, vertice):
    'Barrido en profundidad del grafo'
    while vertice is not None:
        if not vertice.visitado:
            vertice.visitado = True
...
def dijkstra_distancia_antena(grafo, origen, destino):
    '''Dijkstra para hallar el camino mas corto'''
    no_visitados = Heap(tamanio_grafo(grafo))
    camino = Pila()
    aux = grafo.inicio
    ...
    while not heap_vacio(no_visitados):
        ...
        aux = dato[1][0].adyacentes.inicio
        while aux is not None:
            pos = busqueda_heap_antena(no_visitados, aux.destino)
            if no_visitados.vector[pos][0] > dato[0] + aux.info:
                no_visitados.vector[pos][1][1] = dato[1][0].info.nombre
                cambiar_prioridad(no_visitados, pos, dato[0] + aux.info)
            aux = aux.sig
...
def prim_arq(grafo):
    ...
    bosque = []
    aristas = Heap(tamanio_grafo(grafo) ** 2)
    adyac = buscar_vertice_maravilla(grafo, 'M1')
...
def prim_antena(grafo):
    ...
    while len(bosque) // 2 < tamanio_grafo(grafo) and not heap_vacio(aristas):
        ...
        if len(bosque) == 0 or ((dato[1][0] not in bosque) ^ (dato[1][1] not in bosque)):
            bosque += dato[1]
            destino = buscar_vertice_nombre(grafo, dato[1][1])
            adyac = destino.adyacentes.inicio
            while adyac is not None:
                arribo_heap(aristas, [destino.info, adyac.destino], adyac.info)
                adyac = adyac.sig
...
def kruskal_antena(grafo):
    '''Algoritmo de Kruskal para hallar el árbol de expansión mínimo'''
    bosque = []
    aristas = Heap(tamanio_grafo(grafo) ** 2)
    aux = grafo.inicio
    while aux is not None:
        bosque.append([aux.info.nombre])
        adyac = aux.adyacentes.inicio
        while adyac is not None:
            arribo_heap(aristas, [aux.info.nombre, adyac.destino], adyac.info)
            adyac = adyac.sig
        aux = aux.sig
    while len(bosque) > 1 and not heap_vacio(aristas):
        dato = atencion_heap(aristas)
...
def existe_paso_antena(grafo, origen, destino):
    '''Barrido en profundidad del grafo'''
    resultado = False
    if origen.visitado == False:
        ...
        vadyacentes = origen.adyacentes.inicio
        while vadyacentes is not None and not resultado:
            adyacente = buscar_vertice_nombre(grafo, vadyacentes.destino)
            if adyacente.info == destino.info:
                return True
            elif not adyacente.visitado:
                resultado = existe_paso_antena(grafo, adyacente, destino)
            vadyacentes = vadyacentes.sig
    return resultado
